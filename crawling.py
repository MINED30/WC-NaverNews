    ...
    def crawling(self):
        ...
        while page <= maxpage_t:
            ...
            if int((page-1)/10) % 10 == 0:
                print(f"page {int((page-1)/10)} crawled")        
        ...
        try :
            ...
            print(len(self.result['title']),len(self.result['source']),len(self.result['contents']),len(self.result['link']))
            print("only save 'title' & 'contents'")
            df = pd.DataFrame()
            df['title'] = self.result['title']
            df['contents'] = self.result['contents']
            df = df[~df.duplicated()].reset_index(drop=True)
            return df
...
if __name__=="__main__":
        ...
        s_date = input_("검색기간 시작일자 (입력양식:2021.10.26)\n입력 : ")
        e_date = input_("검색기간 종료일자 (입력양식:2021.10.26)\n입력 : ")
